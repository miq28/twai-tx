...
namespace
{
    ...
    bool parseCommand(char *buf, Command &cmd)
    {
        ...
        if (strncmp(buf, "apssid ", 7) == 0)
        {
            cmd.type = CMD_SET_AP_SSID;
            strcpy(cmd.str, buf + 7);
            return true;
        }
        ...
        {
            cmd.type = CMD_SET_AP_PASS;
            strcpy(cmd.str, buf + 7);
            return true;
        }

        if (strncmp(buf, "stassid ", 8) == 0)
        {
            cmd.type = CMD_SET_STA_SSID;
            strcpy(cmd.str, buf + 8);
            return true;
        }

        if (strncmp(buf, "stapass ", 8) == 0)
        {
            cmd.type = CMD_SET_STA_PASS;
            strcpy(cmd.str, buf + 8);
            return true;
        }
    ...
    void handleCommand(const Command &cmd)
    {
        ...
        case CMD_SET_AP_SSID:
            ...
            break;
        case CMD_SET_AP_PASS:
            changePrefsString("AP_PASS", cmd.str);
            break;
        case CMD_SET_STA_SSID:
            changePrefsString("STA_SSID", cmd.str);
            break;
...
void cliProcessByte(uint8_t b)
{
    ...
    if (c == '\n' || c == '\r')
    {
        ...
        Command cmd = {};
        if (parseCommand(lineBuffer, cmd))
            handleCommand(cmd);
        lineIndex = 0;
    }
