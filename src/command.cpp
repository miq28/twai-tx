...
namespace
{
    ...
    const CommandInfo commandTable[] = {
        ...
        {"mode elm327", "m5", "elm327 mode"},
        {"format", "", "set analyzer format (ascii/binary)"},
        {"filter", "", "set analyzer filter (off / ID in hex)"}};
    ...
    bool parseCommand(char *buf, Command &cmd)
    {
        ...
        if (strcmp(buf, "mode elm327") == 0 || strcmp(buf, "m5") == 0)
        {
            cmd.type = CMD_SET_MODE;
            strcpy(cmd.str, "elm327");
            return true;
        }
    ...
    void handleCommand(const Command &cmd)
    {
        ...
        case CMD_SET_MODE:
            ...
            else if (strcmp(cmd.str, "elm327") == 0)
                appState.mode = MODE_ELM327;
            break;
        case CMD_SET_BAUD:
...
void dispatchByte(InputContext &, uint8_t b)
{
    ...
    else if (appState.mode != MODE_SAVVYCAN && (b == 0xE7 || b == 0xF1))
    {
        appState.mode = MODE_SAVVYCAN;
